...
DrawingBoard.Board.prototype = {
	...
	resizeCss: function(canvasWidth1, canvasHeight1){
	...
	draw: function() {		
		...
		if (this.isDrawing) {							
			...
			myCords.push(this.coords.current);
			//myCordsMid.push(currentMid);
			this.ctx.beginPath();
	...
	_onInputStart: function(e, coords) {
		...
		var a = []		
		a.push(this.ctx.lineWidth); 	
	    a.push(this.opts.eraserColor);
		a.push(this.mode);
		a.push(this.color);
		a.push(drawHistory.length);
		drawModes.push(a);
	...
	//Logic to store drawing coordinates in array with settings for draw-mode
	...
	storeLocal: function(){		
		...
		for (var a = 0; a < drawHistory.length;a++){
			myCords = drawHistory[a];

			var drawMode = drawModes[a];
			//console.log(drawMode[0]);
			drawMode[0] = 50;

			this.dom.$cursor.css({ width: drawMode[0] + 'px', height: drawMode[0] + 'px' });	

			for (var x = 0; x < myCords.length;x++){			
				...
				this.ctx.beginPath();
				this.ctx.moveTo(myCords[x].x-250,myCords[x].y);
				this.ctx.quadraticCurveTo(myCords[x].x-250, myCords[x].y, myCords[x-skip].x-250, myCords[x-skip].y);			
				this.ctx.stroke();	
			}
		}
		
		var img = dataImage[memDivId-1];
		var currImageData = JSON.parse(localStorage.getItem('editImageData'));
		...
		id:memDivId,
		drawHistory: {
			coordinates: drawHistory,
			modes: drawModes
		}
		});		

		set_local('editImageData',currImageData);
		updateLocal('imageData',currImageData,memDivId-1);
		
		//localStorage.setItem('editImageData', JSON.stringify(currImageData));
		myCords = [];
		drawHistory = [];				
		if (window.requestAnimationFrame) requestAnimationFrame( $.proxy(function() { this.draw(); }, this) );
	}
